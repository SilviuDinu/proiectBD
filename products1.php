...
<p>
    ...
    if ($suma > $row1[2] && $suma > $row1[3]){
        ...
        var_dump($identifier);
        $r17=mysqli_query($con, "SELECT * FROM loggedin WHERE ID = '$identifier'");
        $row17=mysqli_fetch_array($r17);
        mysqli_query($con, "INSERT INTO licitatii (utilizator, suma, telefon) VALUES ('$row17[1]', '$suma', '$telefon11')");
        $_SESSION['suma']=$suma;
    }
    else {
